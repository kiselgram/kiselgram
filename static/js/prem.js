// static/js/prem.js – Kiselgram Premium (full, no sockets)

(function() {
    'use strict';

    console.log('✨ Kiselgram Premium v4.2.1 (full)');
    ...
    async function loadCurrentUser() {
        try {
            ...
            if (data.success && data.user) {
                ...
                updateUI();
                if (data.user.display_name && data.user.bio) localStorage.setItem('profile_completed', 'true');
            }
        } catch (e) { console.error('Failed to load current user', e); }
    }

    function updateUI() {
        const avatar = getEl('menuUserAvatar'); const name = getEl('menuUserName'); const username = getEl('menuUserUsername');
    ...
    async function fetchTypingStatus() {
        ...
        try {
            ...
            if (statusEl && data.typing && data.typing.length > 0) {
                const names = data.typing.map(u => u.name).join(', ');
                statusEl.textContent = names ? `${names} typing...` : '';
                statusEl.classList.add('typing');
            } else {
                if (activeChat.last_seen) statusEl.textContent = formatLastSeen(activeChat.last_seen);
    ...
    async function loadStories() {
        ...
        try {
            ...
            if (d.success) { window.currentStories = d.stories || []; renderStoriesRow(); }
        } catch (e) {}
    }
        ...
        m.innerHTML = `
            <div class="modal-container" style="max-width:400px">
                <div class="modal-header" style="background:linear-gradient(135deg,#fb6340,#2dce89);color:white">
                    <h3>Create Story</h3><button class="modal-close" onclick="this.closest('.modal-overlay').remove()" style="color:white">✕</button>
                </div>
    ...
    window.previewStoryMedia = (i) => {
        const f = i.files[0]; if (!f) return; const p = getEl('storyMediaPreview'); if (!p) return;
    ...
    window.uploadStory = async () => {
        const f = getEl('storyMediaInput')?.files[0]; if (!f) { showToast('Select media', 'error'); return; }
        const fd = new FormData(); fd.append('media', f); fd.append('caption', getEl('storyCaption')?.value || '');
        try {
            const r = await fetch('/api/stories/create', { method: 'POST', body: fd }); const d = await r.json();
            if (d.success) { document.querySelectorAll('.modal-overlay').forEach(m => m.remove()); showToast('Story posted!', 'success'); loadStories(); }
        } catch (e) {}
    };

    window.openStoryViewer = (uid) => {
        const u = window.currentStories.find(s => s.user_id === uid); if (!u) return;
        window.currentStoryUser = u; window.currentStoryIndex = 0; renderStoryViewer();
    };

    function renderStoryViewer() {
        const story = window.currentStoryUser?.stories?.[window.currentStoryIndex]; if (!story) return;
        const v = document.createElement('div'); v.className = 'story-viewer'; v.id = 'storyViewer';
        v.innerHTML = `
        ...
        startProgress(); fetch(`/api/stories/${story.id}/view`, { method: 'POST' });
    }

    function startProgress() {
        if (window.storyProgressInterval) clearInterval(window.storyProgressInterval);
        const b = getEl(`storyProgress${window.currentStoryIndex}`); if (!b) return;
        let w = 0;
    ...
    window.nextStory = () => {
        if (window.currentStoryIndex < window.currentStoryUser.stories.length - 1) {
            window.currentStoryIndex++; getEl('storyViewer')?.remove(); renderStoryViewer();
        } else {
            ...
            if (idx < window.currentStories.length - 1) {
                window.currentStoryUser = window.currentStories[idx + 1]; window.currentStoryIndex = 0;
                getEl('storyViewer')?.remove(); renderStoryViewer();
    ...
    window.closeStoryViewer = () => {
        if (window.storyProgressInterval) clearInterval(window.storyProgressInterval);
        getEl('storyViewer')?.remove(); document.body.style.overflow = ''; loadStories();
    };

    window.likeCurrentStory = async () => {
        const s = window.currentStoryUser?.stories?.[window.currentStoryIndex]; if (!s) return;
        try { const r = await fetch(`/api/stories/${s.id}/like`, { method: 'POST' }); const d = await r.json();
            if (d.success) { s.liked = d.liked; s.like_count = d.like_count; const b = getEl('storyLikeBtn');
    ...
    window.reactToCurrentStory = async (reaction) => {
        const s = window.currentStoryUser?.stories?.[window.currentStoryIndex]; if (!s) return;
        try { await fetch('/api/stories/' + s.id + '/reaction', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ reaction }) }); }
        catch (e) {}
    };

    window.deleteCurrentStory = async () => {
        if (!confirm('Delete?')) return; const s = window.currentStoryUser?.stories?.[window.currentStoryIndex]; if (!s) return;
        try { const r = await fetch(`/api/stories/${s.id}`, { method: 'DELETE' });
            if (r.ok) {
                window.currentStoryUser.stories.splice(window.currentStoryIndex, 1);
                if (window.currentStoryUser.stories.length === 0) { /* remove user from currentStories */ }
                else { if (window.currentStoryIndex >= window.currentStoryUser.stories.length) window.currentStoryIndex = window.currentStoryUser.stories.length - 1;
                    getEl('storyViewer')?.remove(); renderStoryViewer(); }
                closeStoryViewer();
            }
        } catch (e) {}
    ...
    window.sendStoryReply = () => {
        const m = getEl('storyReplyInput')?.value.trim(); if (!m) return;
        openChat('personal', window.currentStoryUser.user_id);
        setTimeout(() => { if (DOM.messageInput) { DOM.messageInput.value = m; DOM.sendBtn.disabled = false; } }, 500);
        closeStoryViewer();
    };
    ...
    async function refreshMessages() {
        ...
        if (newHash !== cachedMessages.get(cacheKey)) {
            cachedMessages.set(cacheKey, newHash); renderMessages(msgs); applyWallpaper();
        }
    }
    ...
    function applyWallpaper() {
        if (!activeChat) return; const c = DOM.messagesContainer; if (!c) return;
    ...
    window.openChat = async (type, id) => {
        ...
        await loadChatInfo(type, id); await loadMessages(type, id, true); applyWallpaper();
        ...
        if (type === 'personal') {
            try {
                ...
                if (u) {
                    ...
                    if (DOM.chatHeaderAvatar) { DOM.chatHeaderAvatar.textContent = u.username[0].toUpperCase(); DOM.chatHeaderAvatar.className = 'chat-header-avatar personal'; }
                    activeChat.last_seen = u.last_seen;
                }
            } catch (e) {}
        } else {
            if (DOM.chatHeaderName) DOM.chatHeaderName.textContent = type==='group'?'Group':'Channel';
            if (DOM.chatHeaderAvatar) { DOM.chatHeaderAvatar.textContent = type==='group'?'👥':'📢'; DOM.chatHeaderAvatar.className = `chat-header-avatar ${type}`; }
        }
    }
    ...
    window.openChatCustomization = () => {
        ...
        m.innerHTML = `
            <div class="modal-container" style="max-width:450px">
                <div class="modal-header" style="background:linear-gradient(135deg,#fb6340,#2dce89);color:white"><h3>Customization</h3><button class="modal-close" onclick="this.closest('.modal-overlay').remove()" style="color:white">✕</button></div>
                <div class="modal-body">
                    <h4>Wallpaper</h4>
                    ...
                    <div style="display:flex;gap:8px"><button class="modal-btn modal-btn-secondary" onclick="setTextSize('small')">Small</button><button class="modal-btn modal-btn-secondary" onclick="setTextSize('medium')">Medium</button><button class="modal-btn modal-btn-secondary" onclick="setTextSize('large')">Large</button></div>
                </div>
                <div class="modal-footer">
    ...
    window.setWallpaper = (w) => { if (!activeChat) return; localStorage.setItem(`wallpaper_${activeChat.type}_${activeChat.id}`, w); applyWallpaper(); showToast('Wallpaper updated', 'success'); };
    ...
    async function loadContacts() { const c = getEl('contactsList'); if (!c) return; try { const r = await fetch('/api/contacts'); const d = await r.json(); if (d.success) { c.innerHTML = d.contacts.map(u => `<div class="contact-item" onclick="openChat('personal',${u.id})"><div class="contact-avatar">${u.id}</div><div class="contact-info"><div class="contact-name">${escapeHtml(u.display_name)}</div><div class="contact-info"><div class="contact-username">@${escapeHtml(u.username)}</div></div></div>`).join('') || '<div class="empty-state"><p>No contacts</p></div>'; } } catch (e) {} }
    ...
    // ---- Create Group / Channel (unchanged) ----
    ...
    function urlBase64ToUint8Array(base64) { /* ... */ return []; }
    ...
    // ---- Bot API (Premium) ----
    window.createBot = async (name, usernameBot, description) => { const res = await fetch('/premium/api/bot/create', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({name, username: usernameBot, description}) }); const data = await res.json(); if (data.success) { alert(`Bot created! Token: ${data.bot.token}`); loadBotsList(); } else { alert(data.error || 'Failed'); } };
    async function loadBotsList() { const container = getEl('botsList'); if (!container) return; try { const res = await fetch('/premium/api/bot/list'); const data = await res.json(); if (data.success && data.bots) { container.innerHTML = data.bots.map(bot => `<div class="bot-item"><div class="bot-info"><div class="bot-avatar">🤖</div><div class="bot-details"><h5>${escapeHtml(bot.name)}</h5><p>@${escapeHtml(bot.username)}</p></div></div><div class="bot-token">Token: ${escapeHtml(bot.token||'••••••••')}</div><div class="bot-actions"><button class="btn-small" onclick="regenerateBotToken(${bot.id})">🔄</button><button class="btn-small btn-danger" onclick="deleteBot(${bot.id})">🗑️</button></div></div>`).join('') || '<p>No bots created yet.</p>'; } } catch (e) {} }
    window.regenerateBotToken = async (botId) => { if (!confirm('Regenerate token?')) return; try { const res = await fetch(`/premium/api/bot/${botId}/regenerate-token`, { method:'POST' }); const data = await res.json(); if (data.success) { alert(`New token: ${data.token}`); loadBotsList(); } else { alert(data.error || 'Failed'); } } catch (e) {} };
    window.deleteBot = async (botId) => { if (!confirm('Delete bot?')) return; try { const res = await fetch(`/premium/api/bot/${botId}`, { method:'DELETE' }); const data = await res.json(); if (data.success) { loadBotsList(); showToast('Bot deleted','success'); } else { alert(data.error || 'Failed'); } } catch (e) {} };
    document.addEventListener('DOMContentLoaded', () => { if (window.currentUserId) setTimeout(loadBotsList, 1000); });
})();
...
(function() {
    ...
    function renderStoryViewer() { const story = window.currentStoryUser?.stories?.[window.currentStoryIndex]; if (!story) return; const v = document.createElement('div'); v.className = 'story-viewer'; v.id = 'storyViewer'; v.innerHTML = `<div class="story-viewer-header"><div class="story-progress-container">${window.currentStoryUser.stories.map((_,i)=>`<div class="story-progress-bar-container"><div class="story-progress-bar ${i<window.currentStoryIndex?'completed':''}" id="storyProgress${i}" style="background:linear-gradient(90deg,#fb6340,#2dce89)"></div></div>`).join('')}</div><div class="story-viewer-user"><div class="story-viewer-avatar" style="background:linear-gradient(135deg,#fb6340,#2dce89)">${window.currentStoryUser.avatar_letter}</div><div class="story-viewer-info"><span class="story-viewer-name">${escapeHtml(window.currentStoryUser.display_name)}</span><span class="story-viewer-time">${formatTime(story.created_at)}</span></div></div><button class="story-viewer-close" onclick="closeStoryViewer()">✕</button></div><div class="story-viewer-content">${story.media_type==='video'?`<video src="${story.media_url}" autoplay loop muted></video>`:`<img src="${story.media_url}">`}${story.caption?`<div class="story-caption">${escapeHtml(story.caption)}</div>`:''}</div><div class="story-nav"><div class="story-nav-left" onclick="previousStory()"></div><div class="story-nav-right" onclick="nextStory()"></div></div>`; document.body.appendChild(v); document.body.style.overflow = 'hidden'; startProgress(); fetch(`/api/stories/${story.id}/view`, { method:'POST' }); }
    function startProgress() { if (window.storyProgressInterval) clearInterval(window.storyProgressInterval); const b = getEl(`storyProgress${window.currentStoryIndex}`); if (!b) return; let w = 0; window.storyProgressInterval = setInterval(() => { w += 1; b.style.width = w + '%'; if (w >= 100) { clearInterval(window.storyProgressInterval); nextStory(); } }, 50); }
    window.nextStory = () => { if (window.currentStoryIndex < window.currentStoryUser.stories.length-1) { window.currentStoryIndex++; getEl('storyViewer')?.remove(); renderStoryViewer(); } else { const idx = window.currentStories.findIndex(s => s.user_id===window.currentStoryUser.user_id); if (idx < window.currentStories.length-1) { window.currentStoryUser = window.currentStories[idx+1]; window.currentStoryIndex = 0; getEl('storyViewer')?.remove(); renderStoryViewer(); } else { closeStoryViewer(); } } };
    window.previousStory = () => { if (window.currentStoryIndex > 0) { window.currentStoryIndex--; getEl('storyViewer')?.remove(); renderStoryViewer(); } else { const idx = window.currentStories.findIndex(s => s.user_id===window.currentStoryUser.user_id); if (idx > 0) { window.currentStoryUser = window.currentStories[idx-1]; window.currentStoryIndex = window.currentStoryUser.stories.length-1; getEl('storyViewer')?.remove(); renderStoryViewer(); } } };
    ...
    window.likeCurrentStory = async () => { const s = window.currentStoryUser?.stories?.[window.currentStoryIndex]; if (!s) return; try { const r = await fetch(`/api/stories/${s.id}/like`, { method:'POST' }); const d = await r.json(); if (d.success) { s.liked = d.liked; s.like_count = d.like_count; const b = getEl('storyLikeBtn'); if (b) b.innerHTML = `${d.liked?'❤️':'🤍'} <span>${d.like_count}</span>`; } } catch (e) {} };
    window.deleteCurrentStory = async () => { if (!confirm('Delete?')) return; const s = window.currentStoryUser?.stories?.[window.currentStoryIndex]; if (!s) return; try { const r = await fetch(`/api/stories/${s.id}`, { method:'DELETE' }); if (r.ok) { window.currentStoryUser.stories.splice(window.currentStoryIndex, 1); if (window.currentStoryUser.stories.length===0) { const idx = window.currentStories.findIndex(x => x.user_id===window.currentStoryUser.user_id); if (idx>-1) window.currentStories.splice(idx,1); closeStoryViewer(); } else { if (window.currentStoryIndex >= window.currentStoryUser.stories.length) window.currentStoryIndex = window.currentStoryUser.stories.length-1; getEl('storyViewer')?.remove(); renderStoryViewer(); } } } catch (e) {} };
    window.sendStoryReply = () => { const m = getEl('storyReplyInput')?.value.trim(); if (!m) return; openChat('personal', window.currentStoryUser.user_id); setTimeout(() => { if (DOM.messageInput) { DOM.messageInput.value = m; DOM.sendBtn.disabled = false; } }, 500); closeStoryViewer(); };
    ...
    function renderChatList(chats) { const c = DOM.chatList; if (!c) return; if (!chats?.length) { c.innerHTML = '<div class="empty-state"><div class="empty-icon">💬</div><p>No chats</p></div>'; return; } c.innerHTML = chats.map(chat => { const active = window.activeChat?.type===chat.type && window.activeChat?.id===chat.id; return `<div class="chat-item ${active?'active':''}" data-chat-type="${chat.type}" data-chat-id="${chat.id}" onclick="openChat('${chat.type}',${chat.id})"><div class="chat-avatar ${chat.type} ${chat.has_story?'has-story':''}">${chat.avatar_url?`<img src="${chat.avatar_url}">`:(chat.avatar||'?')}${chat.type==='personal'&&chat.is_online?'<span class="online-indicator"></span>':''}</div><div class="chat-info"><div class="chat-name-row"><span class="chat-name">${escapeHtml(chat.name)}</span><span class="chat-time">${chat.timestamp||''}</span></div><div class="chat-preview"><span>${escapeHtml(chat.last_message||'')}</span>${chat.unread_count>0?`<span class="unread-badge">${chat.unread_count}</span>`:''}</div></div></div>`; }).join(''); }
    ...
    window.openChatCustomization = () => { const m = document.createElement('div'); m.className = 'modal-overlay'; m.innerHTML = `<div class="modal-container" style="max-width:450px"><div class="modal-header" style="background:linear-gradient(135deg,#fb6340,#2dce89);color:white"><h3>Customization</h3><button class="modal-close" onclick="this.closest('.modal-overlay').remove()" style="color:white">✕</button></div><div class="modal-body"><h4>Wallpaper</h4><div class="wallpaper-options" style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px">${['gradient1','gradient2','gradient3','gradient4','gradient5','default'].map(w => `<div style="aspect-ratio:1;border-radius:8px;border:2px solid var(--border-color);cursor:pointer;background:${w==='default'?'var(--bg-surface)':''}" onclick="setWallpaper('${w}')">${w==='default'?'Default':''}</div>`).join('')}</div><h4 style="margin-top:20px">Text Size</h4><div style="display:flex;gap:8px"><button class="modal-btn modal-btn-secondary" onclick="setTextSize('small')">Small</button><button class="modal-btn modal-btn-secondary" onclick="setTextSize('medium')">Medium</button><button class="modal-btn modal-btn-secondary" onclick="setTextSize('large')">Large</button></div></div><div class="modal-footer"><button class="modal-btn modal-btn-secondary" onclick="resetChatCustomization()">Reset</button><button class="modal-btn modal-btn-primary" onclick="this.closest('.modal-overlay').remove()" style="background:linear-gradient(135deg,#fb6340,#2dce89)">Done</button></div></div>`; (DOM.modalRoot||document.body).appendChild(m); };
    ...
    window.setTextSize = (size) => { const c = DOM.messagesContainer; if (c) { c.classList.remove('text-small','text-medium','text-large'); c.classList.add(`text-${size}`); localStorage.setItem('chat_text_size', size); } showToast('Text size updated', 'success'); };
    window.resetChatCustomization = () => { if (!window.activeChat) return; localStorage.removeItem(`wallpaper_${window.activeChat.type}_${window.activeChat.id}`); localStorage.removeItem('chat_text_size'); const c = DOM.messagesContainer; if (c) { c.classList.remove('wallpaper-gradient1','wallpaper-gradient2','wallpaper-gradient3','wallpaper-gradient4','wallpaper-gradient5','text-small','text-medium','text-large'); c.classList.add('text-medium'); } showToast('Reset', 'success'); };
    ...
    window.showChatInfo = () => { if (!window.activeChat) return; showToast('Chat info', 'info'); };

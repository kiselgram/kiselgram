...
# Kiselgram SPA API - Complete with Typing Indicators, Status Emoji, Reactions
...
from app.models import (
    ...
    Reaction, Reply, Forward, BlockedUser, Story, StoryView, StoryLike, PushSubscription
)
from app.utils.helpers import get_current_user, get_current_user_id, format_file_size
from app.routes.premium import has_premium_feature
import re
import secrets
import os
import json
...
# Typing cache for indicators
typing_cache = {}  # { "personal_123": { user_id: timestamp } }
...
def global_search():
    try:
        ...
        groups = Group.query.filter(Group.is_public == True, Group.name.ilike(f'%{query}%')).limit(10).all()
        results['groups'] = [{'id': g.id, 'name': g.name, 'description': g.description, 'avatar_url': g.avatar_url} for g in groups]

        channels = Channel.query.filter(Channel.is_public == True, Channel.name.ilike(f'%{query}%')).limit(10).all()
        results['channels'] = [{'id': c.id, 'name': c.name, 'description': c.description, 'avatar_url': c.avatar_url} for c in channels]
...
def get_users():
    try:
        ...
        blocked_ids = get_blocked_user_ids(current_user_id)
        query = User.query.filter(User.id != current_user_id, ~User.id.in_(blocked_ids))
        if search:
...
def get_group_messages(group_id):
    try:
        ...
        after_id = request.args.get('after', 0, type=int)
        limit = request.args.get('limit', 50, type=int)
        messages = Message.query.filter_by(group_id=group_id).filter(Message.id > after_id).order_by(Message.timestamp.asc()).limit(limit).all()
        return jsonify({'success': True, 'messages': [message_to_dict(msg, current_user_id) for msg in messages]})
    except Exception as e:
...
def get_channel_messages(channel_id):
    try:
        ...
        after_id = request.args.get('after', 0, type=int)
        limit = request.args.get('limit', 50, type=int)
        messages = Message.query.filter_by(channel_id=channel_id).filter(Message.id > after_id).order_by(Message.timestamp.asc()).limit(limit).all()
        return jsonify({'success': True, 'messages': [message_to_dict(msg, current_user_id) for msg in messages]})
    except Exception as e:
...
def send_group_message():
    try:
        ...
        if not membership:
            return jsonify({'success': False, 'error': 'Not a member'}), 403
        new_message = Message(content=content, sender_id=current_user_id, group_id=group_id, receiver_id=current_user_id, timestamp=datetime.utcnow())
        db.session.add(new_message)
        db.session.flush()
        if reply_to_id:
            original = Message.query.get(reply_to_id)
...
def send_channel_message():
    try:
        ...
        new_message = Message(content=content, sender_id=current_user_id, channel_id=channel_id, receiver_id=current_user_id, timestamp=datetime.utcnow())
        db.session.add(new_message)
        db.session.commit()
        return jsonify({'success': True, 'message': message_to_dict(new_message, current_user_id)})
    except Exception as e:
...
@spa_bp.route('/typing/<chat_type>/<int:chat_id>', methods=['POST'])
...
@spa_bp.route('/typing/<chat_type>/<int:chat_id>', methods=['GET'])
...
@spa_bp.route('/profile/status', methods=['PUT'])
def update_status_emoji():
    user_id = get_current_user_id()
    if not user_id:
        return jsonify({'success': False, 'error': 'Not authenticated'}), 401
    if not has_premium_feature(user_id, 'custom_status'):
        return jsonify({'success': False, 'error': 'Premium required', 'upgrade': True}), 403
    data = request.get_json()
    emoji = data.get('emoji', '').strip()
    user = User.query.get(user_id)
    user.status_emoji = emoji[:10]
    db.session.commit()
    return jsonify({'success': True, 'emoji': user.status_emoji})
...
def get_reactions(message_id):
    try:
        ...
        user_reaction = Reaction.query.filter_by(message_id=message_id, user_id=current_user_id).first()
        return jsonify({'success': True, 'reactions': list(grouped.values()), 'user_reaction': user_reaction.reaction_type if user_reaction else None})
    except Exception as e:
...
def leave_group(group_id):
    try:
        ...
        if membership and membership.role == 'owner':
            other_members = GroupMember.query.filter_by(group_id=group_id).filter(GroupMember.user_id != current_user_id).first()
            if other_members:
...
def get_stories():
    try:
        ...
        stories = Story.query.filter(Story.user_id.in_(contact_ids), Story.created_at >= cutoff).order_by(Story.created_at.desc()).all()
        user_stories = {}
        ...
        for story in stories:
            ...
            if user.id not in user_stories:
                user_stories[user.id] = {
                    ...
                    'avatar_url': user.avatar_url, 'avatar_letter': user.username[0].upper() if user.username else '?',
                    'stories': [], 'has_unviewed': False
                }
            liked = StoryLike.query.filter_by(story_id=story.id, user_id=current_user_id).first() is not None
            story_data = {
                'id': story.id, 'media_url': f"/uploads/{story.media_path}" if story.media_path else None,
                'media_type': story.media_type, 'caption': story.caption,
                'created_at': story.created_at.isoformat(), 'expires_at': (story.created_at + timedelta(hours=24)).isoformat(),
                'viewed': story.id in viewed_story_ids, 'view_count': StoryView.query.filter_by(story_id=story.id).count(),
                'like_count': StoryLike.query.filter_by(story_id=story.id).count(), 'liked': liked
            }
...
def create_story():
    try:
        ...
        file.save(file_path)
        new_story = Story(user_id=current_user_id, media_path=relative_path, media_type=media_type, caption=caption, created_at=datetime.utcnow())
        db.session.add(new_story)
        db.session.commit()
        return jsonify({'success': True, 'story': {'id': new_story.id, 'media_url': f"/uploads/{relative_path}", 'media_type': media_type, 'caption': caption}})
    except Exception as e:
        db.session.rollback()
...
def create_group():
    try:
        ...
        new_group = Group(name=name, description=description, owner_id=current_user_id, is_public=is_public, invite_link=invite_link, created_at=datetime.utcnow())
        ...
        db.session.commit()
        return jsonify({'success': True, 'group': {'id': new_group.id, 'name': new_group.name, 'avatar_url': new_group.avatar_url}})
    except Exception as e:
        db.session.rollback()
...
def get_group(group_id):
    try:
        ...
        for m in GroupMember.query.filter_by(group_id=group_id).all():
            ...
            if user:
                members.append({'id': user.id, 'username': user.username, 'display_name': user.display_name or user.username, 'avatar_url': user.avatar_url, 'role': m.role})
        return jsonify({'success': True, 'group': {'id': group.id, 'name': group.name, 'description': group.description, 'avatar_url': group.avatar_url, 'is_public': group.is_public, 'owner_id': group.owner_id, 'member_count': len(members), 'members': members, 'is_member': membership is not None, 'role': membership.role if membership else None}})
    except Exception as e:
...
def create_channel():
    try:
        ...
        invite_link = secrets.token_urlsafe(16)
        new_channel = Channel(name=name, description=description, owner_id=current_user_id, is_public=is_public, invite_link=invite_link, created_at=datetime.utcnow())
        ...
        db.session.commit()
        return jsonify({'success': True, 'channel': {'id': new_channel.id, 'name': new_channel.name, 'avatar_url': new_channel.avatar_url}})
    except Exception as e:
        db.session.rollback()
...
def forward_message():
    try:
        ...
        new_message = Message(content=content, sender_id=current_user_id, has_attachment=original.has_attachment, file_name=original.file_name, file_path=original.file_path, file_type=original.file_type, file_size=original.file_size, timestamp=datetime.utcnow())
        ...
        db.session.add(new_message)
        db.session.flush()
        db.session.add(Forward(original_message_id=original_id, forwarded_message_id=new_message.id, forwarded_by_id=current_user_id, original_sender_name=original.sender.username if original.sender else 'Unknown'))
        db.session.commit()
        return jsonify({'success': True, 'message': message_to_dict(new_message, current_user_id)})
    except Exception as e:
        db.session.rollback()
...
def check_premium(user_id):
    try:
        ...
        if not user:
            return jsonify({'success': False, 'error': 'User not found'}), 404
        return jsonify({'success': True, 'is_premium': getattr(user, 'is_premium', False), 'premium_since': user.premium_since.isoformat() if getattr(user, 'premium_since', None) else None, 'premium_expires': user.premium_expires_at.isoformat() if getattr(user, 'premium_expires_at', None) else None})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500
...
@spa_bp.route('/sync_messages', methods=['POST'])
def sync_messages():
    try:
        current_user_id = get_current_user_id()
        if not current_user_id:
        ...
        for msg in messages:
            ...
            if not content or not target_type or not target_id:
                continue
            new_msg = Message(content=content, sender_id=current_user_id, timestamp=datetime.utcnow())
            if target_type == 'personal':
                new_msg.receiver_id = target_id
            elif target_type == 'group':
                new_msg.group_id = target_id
                new_msg.receiver_id = current_user_id
            elif target_type == 'channel':
                new_msg.channel_id = target_id
                new_msg.receiver_id = current_user_id
            else:
                continue
            db.session.add(new_msg)
            db.session.flush()
            synced.append({'temp_id': temp_id, 'server_id': new_msg.id, 'message': message_to_dict(new_msg, current_user_id)})
        db.session.commit()
...
@spa_bp.route('/push/subscribe', methods=['POST'])
def push_subscribe():
    current_user_id = get_current_user_id()
    if not current_user_id:
        return jsonify({'success': False}), 401
    ...
    existing = PushSubscription.query.filter_by(user_id=current_user_id, endpoint=subscription['endpoint']).first()
    if not existing:
        new_sub = PushSubscription(user_id=current_user_id, endpoint=subscription['endpoint'], p256dh=subscription['keys']['p256dh'], auth=subscription['keys']['auth'])
...
def send_push_notification(user_id, title, body, url='/'):
    ...
    vapid_claims = {"sub": "mailto:admin@kiselgram.com"}
    for sub in subs:
        try:
            webpush(subscription_info={"endpoint": sub.endpoint, "keys": {"p256dh": sub.p256dh, "auth": sub.auth}}, data=json.dumps({"title": title, "body": body, "url": url}), vapid_private_key=vapid_private_key, vapid_claims=vapid_claims)
        except WebPushException as e:
            if e.response.status_code in [404, 410]:
                db.session.delete(sub)
    db.session.commit()

def register_spa_bp(app: Flask) -> None:
